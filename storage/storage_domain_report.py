...
------------------------
Reports storage domain (vault) utilization summary across all
Helios-connected clusters.

Storage domains (formerly "Viewboxes") partition raw storage capacity
on Cohesity nodes. Each domain has its own dedup/compression settings,
encryption policy, and storage tier assignments.

Outputs an Excel workbook — one row per storage domain per cluster.

API endpoints used:
  Cluster list:     GET  https://helios.cohesity.com/mcm/clusters/connectionStatus
  Storage domains:  GET  https://helios.cohesity.com/v2/storage-domains
                         ?includeStats=true
...
def get_storage_domains(api_key: str, cluster_id: int) -> list:
    url    = f"https://{HELIOS_HOST}/v2/storage-domains"
...
def write_excel(rows: list, output_path: str):
    ...
    print(f"    Storage domains: {len(rows)}")
...
def main():
    parser = argparse.ArgumentParser(
        description=f"Cohesity storage domain report v{__version__}",
    ...
    for c in clusters:
        cname = c["name"]
        cid   = c["clusterId"]
        print(f"\n[*] Fetching storage domains: {cname}")
