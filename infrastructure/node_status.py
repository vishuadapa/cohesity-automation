...
Usage:
  ...
  python3 node_status.py --ca-bundle /path/to/ca.crt  # corporate proxy cert
...
def write_excel(node_rows: list, disk_rows: list, output_path: str):
    ...
    for row in node_rows:
        ws_n.append([_safe_cell(row.get(h, "")) for h in node_headers])
    style_ws(ws_n)
    auto_fit(ws_n)
    ...
    for row in disk_rows:
        ws_d.append([_safe_cell(row.get(h, "")) for h in disk_headers])
    style_ws(ws_d)
    auto_fit(ws_d)
