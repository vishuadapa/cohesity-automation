"""
Cohesity MCP Server v4 — cluster-direct, with secure credential storage
Personal SE demo tool: monitor -> report -> act (with confirm pattern)

v4 = v2 + secure credential storage. Instead of pasting your API key into
claude_desktop_config.json in plain text, v4 stores it in your operating
system's credential lockbox:

  macOS    -> Keychain
  Windows  -> Credential Manager
  Linux    -> Secret Service (GNOME Keyring / KWallet)

One-time setup (interactive, secrets are never echoed to the screen):
    python3 cohesity_mcp_v4.py setup

Other commands:
    python3 cohesity_mcp_v4.py show     # view stored config (key is masked)
    python3 cohesity_mcp_v4.py test     # connect to the cluster and verify auth
    python3 cohesity_mcp_v4.py clear    # remove stored credentials
    python3 cohesity_mcp_v4.py          # run the MCP server (default)

Transports:
  stdio (default, for Claude Desktop):    python cohesity_mcp_v4.py
  streamable HTTP (for Copilot/VS Code):  MCP_TRANSPORT=http python cohesity_mcp_v4.py

Configuration resolution order (per setting): environment variable first,
then the OS lockbox. Env vars still work exactly as in v2, so existing
configs keep working — but with v4 you no longer need any secrets in env.

Env vars (all optional once `setup` has been run):
...
# ---------------------------------------------------------------------------
# Secure credential storage (OS lockbox via `keyring`)
# ---------------------------------------------------------------------------

# All values live under one keyring "service" name; each setting is an entry.
SERVICE_NAME = "cohesity-mcp"
_LOCKBOX_KEYS = ("cluster", "api_key", "username", "password", "domain", "verify_ssl")
...
def _resolve(env_name: str, lockbox_key: str, default: str = "") -> str:
    """Env var wins (v2 compatibility); otherwise read from the OS lockbox."""
    return os.environ.get(env_name) or _lockbox_get(lockbox_key) or default
...
CLUSTER = _resolve("COHESITY_CLUSTER", "cluster")
API_KEY = _resolve("COHESITY_API_KEY", "api_key")
USERNAME = _resolve("COHESITY_USERNAME", "username")
PASSWORD = _resolve("COHESITY_PASSWORD", "password")
DOMAIN = _resolve("COHESITY_DOMAIN", "domain", "LOCAL")
VERIFY_SSL = _resolve("COHESITY_VERIFY_SSL", "verify_ssl", "false").lower() == "true"
...
# ---------------------------------------------------------------------------
# Setup / management CLI (setup | show | test | clear)
# ---------------------------------------------------------------------------

_USAGE = f"""Cohesity MCP Server v{__version__} — secure credential storage edition

Usage:
  python3 cohesity_mcp_v4.py setup    One-time interactive setup. Stores your
                                      cluster address and API key (or username/
                                      password) in the OS lockbox — macOS
                                      Keychain / Windows Credential Manager.
  python3 cohesity_mcp_v4.py show     Show stored configuration (secrets masked).
  python3 cohesity_mcp_v4.py test     Connect to the cluster and verify auth.
  python3 cohesity_mcp_v4.py clear    Remove all stored credentials.
  python3 cohesity_mcp_v4.py          Run the MCP server (used by Claude Desktop).
"""
...
def _cmd_setup() -> None:
    _require_keyring()
    print(f"Cohesity MCP v{__version__} — one-time setup")
    ...
    verify = input("Verify SSL certificates? (say n for lab/self-signed) [y/N]: ").strip().lower()
    verify_ssl = "true" if verify in ("y", "yes") else "false"

    _lockbox_set("cluster", cluster)
    _lockbox_set("verify_ssl", verify_ssl)
    if api_key:
        _lockbox_set("api_key", api_key)
        for key in ("username", "password", "domain"):
            _lockbox_delete(key)
        auth_desc = f"API key {_mask(api_key)}"
    else:
        _lockbox_set("username", username)
        _lockbox_set("password", password)
        _lockbox_set("domain", domain)
        _lockbox_delete("api_key")
        auth_desc = f"username '{username}' (domain {domain})"

    print(f"\nSaved. Cluster: {cluster} | Auth: {auth_desc} | Verify SSL: {verify_ssl}")
    print(f"Stored in {_backend_name()} under service '{SERVICE_NAME}'.")
    print("\nNext steps:")
    print("  1. Verify the connection:   python3 cohesity_mcp_v4.py test")
    print("  2. Point Claude Desktop at this script — no secrets needed in the config:")
    print('''
     {
       "mcpServers": {
         "cohesity": {
...
def _cmd_show() -> None:
    ...
    print(f"Cluster         : {_lockbox_get('cluster') or '(not set)'}")
    api_key = _lockbox_get("api_key")
    if api_key:
        print(f"Auth method     : API key {_mask(api_key)}")
    elif _lockbox_get("username"):
        pw_state = "(set, hidden)" if _lockbox_get("password") else "(not set)"
        print(f"Auth method     : username '{_lockbox_get('username')}' "
              f"(domain {_lockbox_get('domain') or 'LOCAL'}), password {pw_state}")
    else:
        print("Auth method     : (not set — run: python3 cohesity_mcp_v4.py setup)")
    print(f"Verify SSL      : {_lockbox_get('verify_ssl') or 'false'}")
...
def _cmd_clear() -> None:
    _require_keyring()
    for key in _LOCKBOX_KEYS:
        _lockbox_delete(key)
    print(f"All Cohesity credentials removed from {_backend_name()} "
...
def _cmd_test() -> None:
    if not CLUSTER:
        raise SystemExit(
            "No cluster configured. Run:  python3 cohesity_mcp_v4.py setup"
        )
    ...
    try:
        ...
        if exc.response.status_code in (401, 403):
            raise SystemExit(
                ...
                "re-run: python3 cohesity_mcp_v4.py setup"
            )
        raise SystemExit(f"Cluster returned HTTP {exc.response.status_code}: "
                         f"{exc.response.text[:300]}")
    except httpx.RequestError as exc:
        raise SystemExit(
            f"Could not reach the cluster: {exc}\n"
    ...
    if cmd == "setup":
        ...
        if not CLUSTER:
            raise SystemExit(
                ...
                "Run the one-time setup first:  python3 cohesity_mcp_v4.py setup\n"
